...
BEGIN
  ...
  -- Total customers (zid/salla tenants, excluding any tenant owned by a super_admin)
  SELECT count(*) INTO v_total
  FROM public.settings_workspace w
  WHERE w.platform IN ('zid','salla')
    AND (NOT v_has_range OR (w.created_at >= _from AND w.created_at <= _to))
    AND NOT EXISTS (
      SELECT 1 FROM public.auth_tenant_members m
      JOIN public.auth_user_roles r ON r.user_id = m.user_id
      WHERE m.tenant_id = w.id AND r.role = 'super_admin'::app_role
    );
  ...
  -- Avg response time: average gap between a customer message and the
  -- next ai/agent reply, across ALL tenants (same method as per-store
  -- dashboard_metrics). Cap at 1h; ignore negatives.
  WITH m AS (
